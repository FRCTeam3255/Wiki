# Pit Crew Standards

## 1. Match Threads

After each match, post updates in Slack using the formats below. Paste standalone copies of each message to make them easier to copy for future matches.

### Full Functional Done
Post in Slack once the robot is fully functional after the match:

```
Match # [#] - 🎮✅
```

### Pit Checklist Done
Post in Slack once the pit checklist is complete. Include a picture of the battery:

```
Match # [#] - 🏁✅
```

## 2. Drive Team Communication

The drive team should tell pit mentors about any breaks and issues after each match.

- Post breaks and issues in Slack so nothing is forgotten.

## 3. Drive Team Tips

| Key | Action |
| --- | ------ |
| F1  | Force a Joystick refresh |
